...
```{r temp, fig.width = 5}
temp_plot <- ggplot(ntl_ice %>% filter(!is.na(ave_air_temp_adjusted)),
...
```{r}
comparison_plot <- ggplot(comparison_df %>% filter((name == "pct_change_ice" &
                                   !is.na(value)) | name == "pct_change_temp"),
       aes(x = year)) +
